...
echo " Звіт по журналу реєстрації Актів митного огляду товарів та ТЗ комерційного призначення<p>";
echo " за період з ".$_POST['date_left']." по ".$_POST['date_right']."<p>";
echo " .<p>";



// Оформлено Актів митного огляду
...
// Оформлено Актів митного огляду за напрямком переьіщення
$sql = 'SELECT direction, count(*) cs FROM tzmk_act where date_use between

DATE_FORMAT("'.$_POST['date_left'].'","%y-%m-%d")

AND

DATE_FORMAT("'.$_POST['date_right'].'","%y-%m-%d") GROUP BY direction';

$result = mysql_query($sql) or die(mysql_error());

while ($a = mysql_fetch_assoc($result))
{
echo $a["direction"]." - ". $a["cs"]."&nbsp;&nbsp;|&nbsp;&nbsp;";

 }
echo " - Всього оформлено Актів митного огляду в тому числі за напрямком<p>";
...
(st_mku is not null) GROUP BY st_mku LIMIT 10
...
(tzmk_1  is not null) GROUP BY tzmk_1 LIMIT 10
...
(tzmk_2  is not null) GROUP BY tzmk_2 LIMIT 10
...
(tzmk_3 is not null) GROUP BY tzmk_3 LIMIT 10
