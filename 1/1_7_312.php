...
$sql_articles = 'SELECT * FROM tzmk_act where' . $beetwen_cond . ' and st_mku != "" and st_mku != "000" GROUP BY st_mku ';
...
while ($row = mysql_fetch_assoc($sql_articles)) {
    ...
    $rezident = 'select sum(num_protocol) from tzmk_act where sitizen = "так" and st_mku = "' . $row['st_mku'] . '" and ' . $beetwen_cond;
    $rezident = mysql_fetch_row(mysql_query($rezident));

    $not_rezident = 'select sum(num_protocol) from tzmk_act where sitizen = "ні" and st_mku = "' . $row['st_mku'] . '" and ' . $beetwen_cond;
    ...
    $status_penalty = 'select sum(penalty) from tzmk_act where status_penalty = "так" and st_mku = "' . $row['st_mku'] . '" and ' . $beetwen_cond;
...
$subject_sql = 'select * from tzmk_act where ' . $beetwen_cond . ' and st_mku != "" group by subject';
$subject_sql = mysql_query($subject_sql);

while ($row = mysql_fetch_assoc($subject_sql)) {
    $art_arr = [];
    foreach ($count_report_arr as $k => $el) {
        $art_count = 'select sum(num_protocol) from tzmk_act where ' . $beetwen_cond . ' and st_mku = "' . $k . '" and subject = "' . $row['subject'] . '"';
        $art_count = mysql_fetch_row(mysql_query($art_count));
        $art_arr[$k] = $art_count[0];
